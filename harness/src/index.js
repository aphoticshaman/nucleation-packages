...
export class WorldTrajectoryMonitor {
  ...
  async update() {
    ...
    if (data.crypto.error) {
      ...
      if (data.crypto.bitcoin?.prices) {
        ...
        const btcState = this.#detectors.btcRegime.updateBatch(
          btcReturns.map(r => r.value)
        );
      ...
      if (data.crypto.ethereum?.prices) {
        ...
        const ethState = this.#detectors.ethRegime.updateBatch(
          ethReturns.map(r => r.value)
        );
    ...
    if (state.domains.bitcoin) {
      this.#detectors.correlator.updateSource('financial', 
        new Float64Array([
    ...
    if (state.domains.sentiment) {
      this.#detectors.correlator.updateSource('sentiment',
        new Float64Array([
    ...
    if (state.domains.geophysical) {
      this.#detectors.correlator.updateSource('geophysical',
        new Float64Array([
  ...
  generateReport(state) {
    ...
    for (const [domain, data] of Object.entries(state.domains)) {
      const status = data.transitioning || data.failing || data.atRisk ? '🔴' :
                     data.elevated ? '🟡' : '🟢';
      const conf = data.confidence ? `${(data.confidence * 100).toFixed(0)}%` : 'N/A';
      lines.push(`║  ${status} ${domain.padEnd(15)} Conf: ${conf.padEnd(6)} ${this.#formatDomainDetail(domain, data).padEnd(30)}║`);
    }
