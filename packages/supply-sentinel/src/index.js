...
function phaseToLevel(phase) {
  ...
  switch (phase) {
    case Phase.Stable: return RiskLevel.STABLE;
    case Phase.Approaching: return RiskLevel.ELEVATED;
    case Phase.Critical: return RiskLevel.CRITICAL;
    case Phase.Transitioning: return RiskLevel.DISRUPTED;
    default: return RiskLevel.STABLE;
  }
}
...
export class SupplyMonitor {
  ...
  #ensureInit() {
    if (!this.#initialized) throw new Error('SupplyMonitor not initialized. Call init() first.');
  }
  ...
  static async deserialize(json) {
    ...
    const instance = new SupplyMonitor();
...
export async function assess(values, config = {}) {
  const detector = new SupplyMonitor(config);
  await detector.init();
  const state = detector.updateBatch(values);
  return { atRisk: state.atRisk, elevated: state.elevated, level: state.level, confidence: state.confidence };
}
