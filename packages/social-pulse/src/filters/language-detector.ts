...
const LANGUAGE_PROFILES: LanguageProfile[] = [
  ...
  },
  {
    ...
    commonWords: new Set([
      ...
      'porque', 'esta', 'entre', 'cuando', 'muy', 'sin', 'sobre', 'ser', 'tiene', 'también',
    ]),
  },
  {
    code: 'fr',
  ...
  },
  {
    ...
    commonWords: new Set([
      ...
      'auch', 'es', 'an', 'er', 'hat', 'aus', 'bei', 'sind', 'noch', 'nach',
      'wird', 'einer', 'um', 'am', 'haben', 'war', 'wie', 'oder', 'werden', 'dass',
    ]),
  },
  {
    code: 'pt',
  ...
  },
  {
    ...
    name: 'Arabic',
    commonWords: new Set([
      'في', 'من', 'على', 'إلى', 'أن', 'هذا', 'الذي', 'التي', 'هو', 'كان',
      'لم', 'عن', 'مع', 'هي', 'كل', 'لا', 'ما', 'قد', 'بين', 'إن',
    ]),
  ...
  },
  {
    ...
    name: 'Hindi',
    commonWords: new Set([
      'का', 'के', 'को', 'में', 'है', 'और', 'से', 'की', 'हैं', 'पर',
      'इस', 'एक', 'होता', 'भी', 'ने', 'तो', 'या', 'था', 'करने', 'हो',
    ]),
  ...
  },
  {
    ...
    commonWords: new Set([
      ...
      'برای', 'یک', 'هم', 'تا', 'بود', 'خود', 'ها', 'می', 'او', 'شد',
    ]),
  ...
  },
  {
    ...
    name: 'Turkish',
    commonWords: new Set([
      'bir', 've', 'bu', 'için', 'de', 'da', 'ile', 'gibi', 'o', 'en',
      'çok', 'ne', 'daha', 'var', 'olan', 'kadar', 'sonra', 'olarak', 'ancak', 'şey',
      'ise', 'yok', 'göre', 'olan', 'oldu', 'olur', 'olmuş', 'olmak', 'değil', 'mı',
    ]),
  },
  {
    code: 'uk',
    name: 'Ukrainian',
    commonWords: new Set([
      'і', 'в', 'не', 'на', 'що', 'з', 'як', 'це', 'він', 'та',
      'до', 'є', 'але', 'за', 'від', 'його', 'по', 'вона', 'про', 'так',
    ]),
    scriptRanges: [[0x0400, 0x04FF]], // Cyrillic
  },
];
...
export class LanguageDetector implements PostFilter {
   ...
   */
  detect(text: string): { language: string; confidence: number; alternatives: Array<{ code: string; score: number }> } {
      ...
      for (const profile of LANGUAGE_PROFILES) {
        ...
        for (const [start, end] of profile.scriptRanges) {
          if (code >= start && code <= end) {
            charCounts.set(
              profile.code,
              (charCounts.get(profile.code) ?? 0) + 1
            );
            break;
          }
        }
