...
async function main() {
  ...
  for (const event of events) {
    ...
    if (state.threatLevel !== lastLevel) {
      ...
      console.log(`[${time}] Threat Level: ${lastLevel || 'init'} → ${state.threatLevel.toUpperCase()}`);
      console.log(`         Confidence: ${(state.confidence * 100).toFixed(1)}%, Variance: ${state.variance.toFixed(4)}`);
