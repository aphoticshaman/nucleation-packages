...
const SAMPLE_BTC_PRICES = [
  ...
  67234, 68891, 67102, 69543, 66892, 68123, 66234, 69102, 67543, 70765,
  68234, 66654, 69123, 67543, 70987, 68234, 66102, 69234, 67543, 71102,
  69234, 67102, 70543, 68234, 66102, 69543, 67234, 70102, 68543, 66876,
  69234, 67102, 70234, 68543, 66876, 69234, 67102, 70543, 68876, 66234,
  68102, 70234, 67543, 69876, 67234, 70102, 68543, 66876, 69234, 71102,
  // Phase 2: Calm before storm - LOW VOLATILITY (days 51-70)  
  70234, 70312, 70287, 70298, 70276, 70301, 70289, 70295, 70282, 70291,
  70288, 70294, 70279, 70286, 70293, 70281, 70290, 70284, 70292, 70287,
  // Phase 3: The storm - HIGH VOLATILITY breakout (days 71-90)
  72234, 68102, 75543, 71876, 78234, 73102, 80543, 76876, 83234, 78102,
  86543, 81234, 89102, 84543, 92876, 87234, 95102, 89543, 98876, 93234
];
...
async function main() {
  ...
  for (const { timestamp, value } of returns) {
    ...
    if (state.regime !== lastRegime) {
      ...
      if (lastRegime !== null) {
        console.log(`${change.date}: ${change.from} → ${change.to} (confidence: ${change.confidence})`);
      }
  ...
  for (const asset of assets) {
    ...
    console.log(`${asset.name.padEnd(10)}: ${state.regime.padEnd(10)} (confidence: ${state.confidence.toFixed(2)}, var: ${state.variance.toFixed(6)})`);
  }
}
