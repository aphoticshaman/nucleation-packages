...
async function main() {
  ...
  prices.forEach((price, i) => {
    ...
    if (state.regime !== lastRegime) {
      console.log(`[${i}] Regime change: ${lastRegime || 'initial'} -> ${state.regime} (confidence: ${state.confidence.toFixed(2)})`);
      lastRegime = state.regime;
    }
    
    // Report warnings
    if (state.isWarning && !state.isShifting) {
      console.log(`[${i}] Warning: Potential regime shift approaching (inflection: ${state.inflection.toFixed(2)})`);
    }
  });
  ...
  for (const sensitivity of ['conservative', 'balanced', 'sensitive']) {
    ...
    console.log(`${sensitivity}: regime=${result.regime}, shifting=${result.shifting}, confidence=${result.confidence.toFixed(2)}`);
  }
}
