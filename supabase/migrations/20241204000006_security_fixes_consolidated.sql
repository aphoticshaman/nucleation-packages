...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles') THEN
    ...
    SELECT
      p.id, p.email, p.full_name, p.skill_level, p.created_at, p.updated_at,
      COALESCE((SELECT COUNT(*) FROM learning_events le WHERE le.user_id = p.id), 0) as event_count
    FROM profiles p WHERE p.role = 'user';
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'profiles')
    ...
    SELECT
      p.id, p.email, p.full_name, p.company, p.created_at, p.updated_at,
      c.tier, c.api_enabled,
      (SELECT COUNT(*) FROM api_keys ak WHERE ak.client_id = c.id AND ak.revoked = false) as active_keys
    FROM profiles p LEFT JOIN clients c ON c.user_id = p.id WHERE p.role = 'enterprise';
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'training_examples') THEN
    ...
    FROM training_examples WHERE quality_score >= 0.5 ORDER BY weight DESC, quality_score DESC;
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nations') THEN
    ...
    FROM nations WHERE transition_risk > 0.6 AND status = 'active' ORDER BY transition_risk DESC;
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'training_examples') THEN
    ...
    FROM training_examples GROUP BY domain ORDER BY example_count DESC;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nations') THEN
    ...
    SELECT
      ...
      CASE
        ...
        ELSE 'STABLE'
      END as risk_level
    FROM nations n WHERE n.status = 'active';
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nations') THEN
    ...
    FROM nations WHERE status IN ('active', 'disputed');
