...
const PRIORITY_COUNTRIES = [
  ...
  'POL', 'SWE', 'BEL', 'ARG', 'NOR', 'AUT', 'IRN', 'THA', 'ARE', 'NGA',
  'ISR', 'ZAF', 'SGP', 'HKG', 'MYS', 'PHL', 'DNK', 'COL', 'PAK', 'CHL',
  'FIN', 'EGY', 'BGD', 'VNM', 'CZE', 'PRT', 'ROU', 'NZL', 'PER', 'IRL',
]
  ...
  for (const code of countries) {
    ...
    try {
      ...
      for (const [indicator, value] of Object.entries(indicators)) {
        ...
        await supabase.from('country_signals').upsert({
          country_code: code,
          country_name: data.Government?.['Country name']?.['conventional short form']?.text || code,
